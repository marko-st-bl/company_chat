...
<c:if test="${not empty requestScope.serviceName}">
	<form method="post"
		action="<c:url value="axis2-admin/updateServiceParameters"/>">
		<input type="hidden" name="axisService"
			value="<c:out value="${requestScope.serviceName}"/>">
		<table summary="main content table" width="100%">
			...
			<c:forEach items="${requestScope.parameters}" var="parameter">
				<tr>
					...
					<td><input type="text"
						name="<c:out value="${requestScope.serviceName}_${parameter.key}"/>"
				...
				<c:forEach items="${requestScope.operations}" var="operation">
					...
					<c:forEach items="${operation.value}" var="parameter">
						<tr>
							<td><c:out value="${parameter.key}" /></td>
							<td><input type="text"
								name="<c:out value="${operation.key}_${parameter.key}"/>"
								value="<c:out value="${parameter.value}"/>" size="50"></td>
						</tr>
					</c:forEach>
				</c:forEach>
